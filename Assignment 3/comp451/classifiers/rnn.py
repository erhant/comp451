...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        # NOTE: We are told to use LSTM depending on cell_type but we do not have LSTM implementation.
        ...
        # for the forward we did: 
        #     affine -> word_embed -> rnn -> temporal_affine
        # for the backward we should do the opposite:
        #     temporal_affine -> rnn -> word_embed -> affine
    ...
    def clip_grad_norm(self, grads, gclip):
        ...
        ###########################################################################
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        # Clip the gradients (lecture09_RNN.pdf, slide 84)
        # threshold is gclip
        clipped_grads = {}
        for key in grads:            
            g = grads[key]
            grad_norm = np.sum(g * g)
            if grad_norm > gclip:
                cg = g * (gclip / grad_norm)
            else:
                cg = g                
    ...
    def sample_greedily(self, features, max_length=30):
        ...
        # (1) #
        word_embed, _ = word_embedding_forward(self._start, W_embed) # <START> token
        ...
        for step in range(max_length):
            ...
            # (4) #
            captions[:, step] = np.argmax(step_scores, axis=1) # this is where we take the word with highest score

            word_embed, _ = word_embedding_forward(captions[:, step], W_embed) # then embed the next word
    ...
    def sample_randomly(self, features, max_length=30):
        ...
        # code is almost the same except step (4)
        ...
        for step in range(max_length):
            ...
            probs = np.exp(step_scores - np.max(step_scores, axis=1, keepdims=True))
            probs /= np.sum(probs, axis=1, keepdims=True)            
            tmptmp = np.random.choice(range(len(probs[0])), size=(1,1), p=probs[0])
            #print("PROBABILITIES:",probs)
            #print("PROBABILITIES SHAPE:",probs.shape)
            #print("RANDOM CHOICE:",tmptmp)
            captions[:, step] = tmptmp
            
            word_embed, _ = word_embedding_forward(captions[:, step], W_embed) # then get the next word
