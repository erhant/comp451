...
def sgd_momentum(w, dw, config=None):
    ...
    v = momentum * v - learning_rate * dw # velocity calculation happens here
    next_w = w + v # actual update happens here
...
def sgd_nesterov_momentum(w, dw, config=None):
    ...
    # we will use the rearranged nesterov as seen on the lecture slides
    learning_rate = config['learning_rate']
    momentum = config['momentum']
    
    v_prev = v     
    v = momentum * v - learning_rate * dw # velocity calculation happens here
    next_w = w + -momentum * v_prev + (1 + momentum) * v # actual update happens here
...
def rmsprop(w, dw, config=None):
    ...
    v = beta * v + (1 - beta) * (dw * dw) # leaky AdaGrad happens here
